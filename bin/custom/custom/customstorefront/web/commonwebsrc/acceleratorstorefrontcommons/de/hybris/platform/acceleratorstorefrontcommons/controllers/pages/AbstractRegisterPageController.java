...
public abstract class AbstractRegisterPageController extends AbstractPageController
{
			...
			final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		...
		if (form.isOtpForRegistrationEnabled())
		{
			data.setVerificationTokenId(form.getVerificationTokenId());
			data.setVerificationTokenCode(form.getVerificationTokenCode());
		}
		try
		{
		...
		{
			return handleTokenOrPasswordVerificationError(model, form, "register.error.otp.token.verify");
		}
		catch (final PasswordPolicyViolationException e)
		{
			return handleTokenOrPasswordVerificationError(model, form, "registration.error.password.violate.policy");
		}
	...
	private String handleTokenOrPasswordVerificationError(final Model model, final RegisterForm form, final String errorMessage)
			throws CMSItemNotFoundException
	{
		form.setTermsCheck(false);
		model.addAttribute(form);
