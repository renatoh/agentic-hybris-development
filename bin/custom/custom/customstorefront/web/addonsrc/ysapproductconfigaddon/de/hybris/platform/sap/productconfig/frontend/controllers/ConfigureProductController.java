...
public class ConfigureProductController extends AbstractProductConfigController
{
	...
	private static final String[] FILTERED_REDIRECT_KEYS = new String[]
	{ SapproductconfigfrontendWebConstants.CONFIG_ATTRIBUTE,
			BindingResult.MODEL_KEY_PREFIX + SapproductconfigfrontendWebConstants.CONFIG_ATTRIBUTE,
			SapproductconfigfrontendWebConstants.PRODUCT_ATTRIBUTE, AbstractPageController.CMS_PAGE_TITLE, GALLERY_IMAGES,
			AbstractPageController.CMS_PAGE_MODEL, AbstractProductConfigController.CMS_PAGE_TYPE,
			GlobalMessages.ERROR_MESSAGES_HOLDER, GlobalMessages.INFO_MESSAGES_HOLDER, GlobalMessages.CONF_MESSAGES_HOLDER,
			AbstractProductConfigController.METATAGS };
			...
			final RedirectAttributes redirectAttributes) throws BusinessException
	{
		...
		{
			this.addFilteredRedirectAttributes(model, redirectAttributes);
			return REDIRECT_PREFIX + ROOT + configData.getKbKey().getProductCode() + SapproductconfigfrontendWebConstants.CONFIG_URL;
		}
	}

	protected void addFilteredRedirectAttributes(final Model model, final RedirectAttributes redirectAttributes)
	{
		model.asMap().entrySet().stream().filter(entry -> Arrays.stream(FILTERED_REDIRECT_KEYS).anyMatch(entry.getKey()::equals))
