...
public class UiStateHandler
{
	...
	{
		final Predicate<GroupType> csticOrConflictGroupType = groupType -> (GroupType.CSTIC_GROUP == groupType)
				|| (GroupType.CONFLICT == groupType);
	...
	public UiGroupData handleAutoExpand(final ConfigurationData configData, final UiStatus uiStatus)
	{
		...
		else
		{
			...
			if (errorCstic != null)
			{
				...
				if (GroupType.CONFLICT == expandedGroup.getGroupType())
				{
	...
	public void handleProductConfigMessages(final List<ProductConfigMessageData> messages, final Model model)
	{
		...
		{
			if (ProductConfigMessageUISeverity.ERROR == message.getSeverity())
			{
				// red message box
				GlobalMessages.addErrorMessage(model, message.getMessage());
			}
			else if (ProductConfigMessageUISeverity.INFO == message.getSeverity())
			{
				// yellow message box
				GlobalMessages.addInfoMessage(model, message.getMessage());
			}
			else if (ProductConfigMessageUISeverity.CONFIG == message.getSeverity())
			{
				// blue message box
	...
	public void compileGroupForDisplay(final ConfigurationData configData, final UiStatus uiStatus)
	{
		...
		if (configData.getCpqAction() != null && ((configData.getCpqAction() == CPQActionType.NAV_TO_CSTIC_IN_CONFLICT)
				|| (configData.getCpqAction() == CPQActionType.NAV_TO_CSTIC_IN_GROUP)))
		{
	...
	static class CsticStatusErrorWarningPredicate implements Predicate<CsticData>
	{
		...
		{
			return (CsticStatusType.ERROR == csticData.getCsticStatus()) || (CsticStatusType.WARNING == csticData.getCsticStatus());
		}
	}
	...
	static class CsticStatusErrorWarningConflictPredicate implements Predicate<CsticData>
	{
		...
		{
			return (CsticStatusType.ERROR == csticData.getCsticStatus()) || (CsticStatusType.WARNING == csticData.getCsticStatus())
					|| (CsticStatusType.CONFLICT == csticData.getCsticStatus());
		}
	}
	...
	static class GroupStatusErrorWarningPredicate implements Predicate<UiGroupData>
	{
		...
		{
			return (GroupStatusType.ERROR == uiGroup.getGroupStatus()) || (GroupStatusType.WARNING == uiGroup.getGroupStatus());
		}
	}
	...
	static class GroupStatusErrorWarningConflictPredicate implements Predicate<UiGroupData>
	{
		...
		{
			return (GroupStatusType.ERROR == uiGroup.getGroupStatus()) || (GroupStatusType.WARNING == uiGroup.getGroupStatus())
					|| (GroupStatusType.CONFLICT == uiGroup.getGroupStatus());
		}
	}
