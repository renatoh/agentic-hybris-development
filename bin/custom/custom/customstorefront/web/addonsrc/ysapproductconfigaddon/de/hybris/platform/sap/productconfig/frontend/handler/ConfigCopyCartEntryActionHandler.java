...
public class ConfigCopyCartEntryActionHandler implements CartEntryActionHandler
{
	...
	protected ConfigurationData copyConfiguration(final OrderEntryData entryToCopy)
	{
		...
		// Cart entry does not exist at this point of time. We don't need to provide it here, 
		// because later we will add the new configuration to the cart and update the configuration source accordingly.
		final String newConfigId = getConfigurationCopyStrategy().deepCopyConfiguration(configId, pCode, null, true, null);
		if (LOG.isDebugEnabled())
		{
			LOG.debug(
