...
<div class="addtocart-component">
        ...
        <%-- NET-8940: "Add to compare" - PDP is the only entry point for this action. Mapped
             outside "/p/" deliberately - see AddToCompareController's class javadoc. --%>
        <c:url value="/compare/add/${fn:escapeXml(product.code)}" var="addToCompareUrl"/>
        <form action="${fn:escapeXml(addToCompareUrl)}" method="post" class="add-to-compare-form">
            <sec:csrfInput/>
            <button type="submit" class="btn btn-default btn-block btn-icon js-add-to-compare">
                <spring:theme code="product.compare.addToCompare"/>
            </button>
        </form>
    </div>
</div>
